...
console.info("iOS: %s; document.%s; %s", isIOS, hiddenKey, visibilityEventName);
...
export class UnmuteHandler {
	...
	constructor(context: AudioContext, silenceSource: string) {
		...
		console.log("binding", START_PLAYBACK_EVENTS);
		...
		console.log("binding statechange");
	...
	private updateContextState() {
		...
		if (
			...
			this.hasMediaPlaybackEventOccurred
		) {
	...
	private updatePlaybackState() {
		...
		console.log(shouldAllowPlayback, this.allowPlayback);
